...
for lr_in in learning_rate_in:
    for lr_var in learning_rate_var:
        for lr_out in learning_rate_out:
            for dc in decay_epsilon:
                savename = 'lr_in_'+str(lr_in)+'-lr_var_'+str(lr_var)+'-lr_out_'+str(lr_out)+'-dc_'+str(dc)
                for rep in range(repetitions):
                    ...
                    qdqn = QDQN(file_name, model, model_target, n_layers, n_holes, qubits, memory_size, [lr_in, lr_var, lr_out], gamma, n_episodes,
                                steps_per_train, soft_weight_update, steps_per_target_update, tau, epsilon_start, epsilon_min,
                                dc, temperature, batch_size, min_size_buffer, max_size_buffer, exploration_strategy)
